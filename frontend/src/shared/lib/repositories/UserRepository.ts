...
export class UserRepository {
  listenUserFavoriteLibraryIds(cb: (ids: string[]) => void) {
    ...
    (async () => {
      ...
      unsub = onSnapshot(qFav, snap => { const ids: string[] = []; if (snap) { snap.forEach(d => { const data = d.data() as { libraryId?: string }; if(data.libraryId) ids.push(data.libraryId); }); } cb(ids); });
    })();
  ...
  async addFavorite(libraryId: string) {
    ...
    const snap = await getDocs(qFav); if(!snap || !snap.empty) return;
  ...
  async updateUserSettings(patch: UserSettingsUpdate): Promise<UserSettings> {
    ...
    if (patch.profile?.displayName) {
      payload.displayName = patch.profile.displayName;
    }
  ...
  listenUserNotifications(cb: (items: NotificationDoc[]) => void) {
    ...
    (async () => {
      ...
      unsub = onSnapshot(qN, snap => { const arr: NotificationDoc[] = []; if (snap) { snap.forEach(d=> { const data = d.data() as { userId: string; type: string; title: string; message: string; read?: boolean; createdAt?: { toMillis?: ()=>number }; data?: Record<string, unknown> }; arr.push({ id: d.id, userId: data.userId, type: data.type, title: data.title, message: data.message, data: data.data, read: !!data.read, createdAt: data.createdAt?.toMillis? new Date(data.createdAt.toMillis()).toISOString(): '' }); }); arr.sort((a,b)=> b.createdAt.localeCompare(a.createdAt)); } cb(arr); });
    })();
    return () => { cancelled = true; if(unsub) unsub(); };
  }
  ...
  async createAccessRequest(libraryId: string, ownerId: string): Promise<string> {
    ...
    try { const qExistingShare = query(collection(db, SHARES), where('libraryId','==', libraryId), where('targetUserId','==', user.uid), limit(1)); const shareSnap = await getDocs(qExistingShare); if(shareSnap && !shareSnap.empty) return shareSnap.docs[0].id; } catch { /* ignore */ }
    const qReq = query(collection(db, ACCESS_REQUESTS), where('libraryId','==', libraryId), where('requesterId','==', user.uid), where('status','==','pending'), limit(1)); const snap = await getDocs(qReq); if(snap && !snap.empty) return snap.docs[0].id;
