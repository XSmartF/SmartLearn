import type { FieldValue, DocumentSnapshot, QueryDocumentSnapshot } from 'firebase/firestore';
...
import type { CreateLibraryInput, CreateCardInput } from '@/shared/types';
import { invalidateCache, cached } from '@/shared/lib/cache';
import {
  serverTimestamp, addDoc, collection, updateDoc, doc,
  getDoc, query, where, getDocs, writeBatch, increment, onSnapshot,
...
function mapDocToLibrary(s: DocumentSnapshot | QueryDocumentSnapshot): LibraryMeta | null {
...
export class LibraryRepository implements ILibraryRepository {
  ...
  async updateLibrary(
    id: string,
    data: { title?: string; description?: string; visibility?: LibraryVisibility; tags?: string[]; subject?: string; difficulty?: string },
  ) {
    const patch: Record<string, string | string[] | LibraryVisibility | FieldValue | undefined> = {};
  ...
  async createCard(input: CreateCardInput) {
    ...
    invalidateCache(`cards:${input.libraryId}`);
    invalidateCache(`library:${input.libraryId}`);
  }
  ...
  async createCardsBulk(libraryId: string, items: { front: string; back: string; domain?: string }[]): Promise<number> {
    ...
    for (let i = 0; i < items.length; i += CHUNK) {
      ...
      for (const it of slice) {
        ...
        batch.set(ref, { libraryId, front: it.front, back: it.back, domain: it.domain ?? null, difficulty: null, createdAt: now, updatedAt: now });
      }
      await batch.commit();
    ...
    try {
      await updateDoc(doc(db, LIBRARIES, libraryId), { cardCount: increment(created), updatedAt: serverTimestamp() });
    } catch { /* ignore */ }
  ...
  async fetchLibrariesByIds(ids: string[]): Promise<LibraryMeta[]> {
    ...
    for (const chunk of chunks) {
      const q = query(collection(db, LIBRARIES), where('__name__', 'in', chunk));
      const snap = await getDocs(q);
      snap.forEach(s => {
        const lib = mapDocToLibrary(s);
        if (lib) results.push(lib);
      });
    }
  ...
  listenUserLibraries(cb: (libs: LibraryMeta[]) => void) {
    ...
    (async () => {
      if (cancelled) return;
      const q = query(collection(db, LIBRARIES), where('ownerId', '==', user.uid));
  ...
  async deleteLibrary(id: string) {
    ...
    const q = query(collection(db, CARDS), where('libraryId', '==', id));
    const snap = await getDocs(q);
    const batch = writeBatch(db);
