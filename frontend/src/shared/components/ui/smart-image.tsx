...
export const SmartImage = React.forwardRef<HTMLImageElement, SmartImageProps>(
  ({ className, imageClassName, fallback, rounded = false, onLoad, onError, ...props }, ref) => {
  const [errored, setErrored] = React.useState(false);

    const radiusClass = typeof rounded === "string" ? rounded : rounded ? "rounded-lg" : "";
    const wrapperClass = cn("relative inline-block", radiusClass, className);
      ...
      <span className={wrapperClass}>
        {!errored && (
          <img
            ...
            className={cn("block w-full h-full", resolvedImageClass)}
          />
        )}
        {errored && (
