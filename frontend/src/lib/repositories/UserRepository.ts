...
import { getDb, getFirebaseAuth } from '@/lib/firebaseClient';
...
export class UserRepository {
  ...
  async addFavorite(libraryId: string) {
    const user = getFirebaseAuth().currentUser; if(!user) throw new Error('Not authenticated');
    const { query, collection, where, getDocs, addDoc, serverTimestamp } = await fs();
  ...
  async removeFavorite(libraryId: string) {
    const user = getFirebaseAuth().currentUser; if(!user) throw new Error('Not authenticated');
    const { query, collection, where, limit, getDocs, deleteDoc, doc } = await fs();
  ...
  async getUserProfile(userId: string): Promise<{ id: string; email?: string; displayName?: string; avatarUrl?: string } | null> {
    return cached([`user:${userId}`], async () => { try { const { getDoc, doc } = await fs(); const snap = await getDoc(doc(db, USERS, userId)); if(!snap.exists()) return null; const d = snap.data() as { email?: string; displayName?: string; avatarUrl?: string }; return { id: snap.id, email: d.email, displayName: d.displayName, avatarUrl: d.avatarUrl }; } catch { return null; } });
  }
  async findUserByEmail(email: string): Promise<{ id: string; email?: string; displayName?: string }[]> {
    if(!email) return []; const { query, collection, where, limit, getDocs } = await fs(); const qUsers = query(collection(db, USERS), where('email','==', email.toLowerCase()), limit(5)); const snap = await getDocs(qUsers); const arr: { id: string; email?: string; displayName?: string }[] = []; snap.forEach(s=> { const d = s.data() as { email?: string; displayName?: string }; arr.push({ id: s.id, email: d.email, displayName: d.displayName }); }); return arr;
  }
  ...
  async markNotificationRead(id: string) { const { updateDoc, doc } = await fs(); await updateDoc(doc(db, NOTIFICATIONS, id), { read: true }); }
  async markAllNotificationsRead() {
    const user = getFirebaseAuth().currentUser; if(!user) throw new Error('Not authenticated');
    const { query, collection, where, limit, getDocs, updateDoc, doc } = await fs();
  ...
  async createNotification(userId: string, input: { type: string; title: string; message: string; data?: Record<string, unknown> }) { const { addDoc, collection, serverTimestamp } = await fs(); await addDoc(collection(db, NOTIFICATIONS), { userId, type: input.type, title: input.title, message: input.message, data: input.data || {}, read: false, createdAt: serverTimestamp() }); }
  async createAccessRequest(libraryId: string, ownerId: string): Promise<string> {
    ...
    const { query, collection, where, limit, getDocs, addDoc, serverTimestamp } = await fs();
  ...
  async actOnAccessRequest(reqId: string, approve: boolean) {
    const { doc, getDoc, addDoc, collection, serverTimestamp, updateDoc } = await fs();
