...
import { ArrowLeft, CheckCircle, Settings, Target, Clock, Zap } from 'lucide-react';
...
export default function TrueFalseSettingsPage() {
  const navigate = useNavigate();
  const [settings, setSettings] = useState<TrueFalseSettings>({
    difficulty: 'easy',
    questionCount: 10,
    timeLimit: undefined,
    showHints: true,
  ...
  const handleStartGame = () => {
    navigate('/games/true-false/play', { state: { settings } });
  };

  const getDifficultyDescription = (difficulty: string) => {
    switch (difficulty) {
      case 'easy':
        return '10 câu hỏi - Thời gian thoải mái';
      case 'medium':
        return '15 câu hỏi - Nhanh hơn một chút';
      case 'hard':
        return '20 câu hỏi - Thử thách tối đa';
      default:
        return '';
    }
  };
    ...
    <div className="container mx-auto p-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <H1 className="mb-0">Cài đặt trò chơi đúng/sai</H1>
        </div>
      ...
      <Card className="mb-6">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold mb-2">Trò chơi đúng/sai</h2>
              <p className="text-muted-foreground mb-4">
                Kiểm tra kiến thức của bạn bằng cách xác định xem các câu phát biểu có đúng hay không.
                Trò chơi này giúp củng cố kiến thức và cải thiện khả năng phân tích.
              </p>
