...
import { ArrowLeft, FileText, Settings, Target, Clock, Zap } from 'lucide-react';

interface FillBlankSettings {
...
export default function FillBlankSettingsPage() {
  const navigate = useNavigate();
  const [settings, setSettings] = useState<FillBlankSettings>({
    difficulty: 'easy',
    questionCount: 10,
    timeLimit: undefined,
    showHints: true,
  ...
  const handleStartGame = () => {
    navigate('/games/fill-blank/play', { state: { settings } });
  };

  const getDifficultyDescription = (difficulty: string) => {
    switch (difficulty) {
      case 'easy':
        return '10 câu hỏi - Từ vựng cơ bản';
      case 'medium':
        return '15 câu hỏi - Từ vựng nâng cao';
      case 'hard':
        return '20 câu hỏi - Từ vựng chuyên sâu';
      default:
        return '';
    }
  };
    ...
    <div className="container mx-auto p-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <H1 className="mb-0">Cài đặt trò chơi điền khuyết</H1>
        </div>
      ...
      <Card className="mb-6">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold mb-2">Trò chơi điền khuyết</h2>
              <p className="text-muted-foreground mb-4">
                Điền từ đúng vào chỗ trống để hoàn thành câu. Trò chơi này giúp củng cố từ vựng
                và cải thiện khả năng hiểu ngữ cảnh.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center gap-2 text-sm">
                  <Target className="h-4 w-4 text-blue-500" />
                  <span>Củng cố từ vựng</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
