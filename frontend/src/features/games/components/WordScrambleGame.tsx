...
import { ArrowLeft, RotateCcw, Shuffle, Target, Trophy, CheckCircle, XCircle, Lightbulb, Loader2, HelpCircle } from 'lucide-react';
...
export default function WordScrambleGame({ difficulty = 'easy' }: WordScrambleGameProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const { cards, loading: cardsLoading, error: cardsError } = useAllGameCards();
  const [gameState, setGameState] = useState<WordScrambleGameState | null>(null);
  ...
  const getNextCard = (state: WordScrambleGameState) => {
    ...
    return {
      ...selectedCard,
      scrambledWord: shuffleWord(selectedCard.back)
    };
  };

  const startGame = (state: WordScrambleGameState): WordScrambleGameState => {
    ...
    return {
      ...
      scrambledWord: nextCard.scrambledWord,
      gameStarted: true,
  ...
  const submitAnswer = (state: WordScrambleGameState, answer: string): WordScrambleGameState => {
  ...
  const nextCard = (state: WordScrambleGameState): WordScrambleGameState => {
  ...
  const resetGame = (cards: GameCard[], difficulty: string): WordScrambleGameState => {
  ...
  const getGameStats = (state: WordScrambleGameState) => {
      ...
      <div className="container mx-auto p-6 max-w-2xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <H1 className="mb-0">Kết quả trò chơi xáo trộn từ</H1>
          </div>
    ...
    <div className="container mx-auto p-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <H1 className="mb-0">Trò chơi xáo trộn từ</H1>
        </div>
        <Badge className={getDifficultyColor()}>
          {gameDifficulty === 'easy' ? 'Dễ' : gameDifficulty === 'medium' ? 'Trung bình' : 'Khó'}
        </Badge>
        ...
        <CardContent className="p-6">
          {gameState.gamePhase === 'waiting' && (
            <div className="text-center py-12">
              ...
              <h2 className="text-2xl font-bold mb-4">Trò chơi xáo trộn từ</h2>
          ...
          {gameState.gamePhase === 'playing' && gameState.currentCard && (
            <div className="text-center py-8">
              <h2 className="text-xl font-bold mb-6">Sắp xếp lại thành từ đúng:</h2>

              <div className="text-lg bg-gray-50 p-4 rounded-lg mb-4">
                {gameState.currentCard.front}
              </div>

              <div className="text-4xl font-bold mb-6 text-purple-600 bg-purple-50 p-6 rounded-lg border-2 border-purple-200">
                {gameState.scrambledWord.split('').map((letter, index) => (
                  <span key={index} className="inline-block mx-1 p-2 bg-white rounded border">
                    {letter}
                  </span>
                ))}
              </div>
          ...
          {gameState.gamePhase === 'result' && gameState.currentCard && (
            <div className="text-center py-8">
              ...
              <div className="bg-purple-50 p-4 rounded-lg mb-4">
                <div className="text-sm text-muted-foreground mb-2">Từ xáo trộn:</div>
              ...
              <div className="flex gap-3 justify-center">
                <Button variant="outline" onClick={() => setInputValue(gameState.currentCard?.back || '')}>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <ul className="text-sm text-muted-foreground space-y-1">
            <li>• Xem câu hỏi và các chữ cái đã được xáo trộn</li>
            <li>• Sắp xếp lại các chữ cái thành từ đúng</li>
